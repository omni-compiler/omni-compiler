program coarray_coarray_mem_get_f
  use xmp_api
  ...
  if(my_image == 2) then
     img_dims(1) = 1
     call xmp_coarray_mem_get(img_dims,v_desc,v1, status)
     call xmp_coarray_mem_get(img_dims,s_desc,s1,5,status)
  end if
...
!     print *,"v=", v
!     print *,"s=", s
     if(v1 == v2 .and. s1(1:5) == s2(1:5)) then
        print *,'PASS'
     else
        print *,'ERROR'
     end if
...
end program coarray_coarray_mem_get_f
