...
extern int access ( char *__name, int __type) __THROW __nonnull ((1));

#ifdef __USE_GNU
/* Test for access to NAME using the effective UID and GID
   (as normal file operations use).  */
extern int euidaccess ( char *__name, int __type)
     __THROW __nonnull ((1));

/* An alias for `euidaccess', used by some other systems.  */
extern int eaccess ( char *__name, int __type)
     __THROW __nonnull ((1));
#endif
...
extern ssize_t __REDIRECT (pread, (int __fd, void *__buf, size_t __nbytes,
				   __off64_t __offset),
			   pread64) ;
extern ssize_t __REDIRECT (pwrite, (int __fd,  void *__buf,
...
/* Write N bytes of BUF to FD at the given position OFFSET without
   changing the file pointer.  Return the number written, or -1.  */
extern ssize_t pwrite64 (int __fd,  void *__buf, size_t __n,
			 __off64_t __offset) ;
# endif
#endif
...
extern int chdir ( char *__path) __THROW __nonnull ((1)) ;
...
extern int execve ( char *__path, char * __argv[],
		   char * __envp[]) __THROW __nonnull ((1));
...
extern ssize_t readlink ( char * __path,
...
extern int symlinkat ( char *__from, int __tofd,
		       char *__to) __THROW __nonnull ((1, 3)) ;

/* Like readlink but a relative PATH is interpreted relative to FD.  */
extern ssize_t readlinkat (int __fd,  char * __path,
			   char * __buf, size_t __len)
     __THROW __nonnull ((2, 3)) ;
#endif

/* Remove the link NAME.  */
extern int unlink ( char *__name) __THROW __nonnull ((1));
...
extern int rmdir ( char *__path) __THROW __nonnull ((1));
...
extern int setlogin ( char *__name) __THROW __nonnull ((1));
#endif
...
extern int revoke ( char *__file) __THROW __nonnull ((1)) ;
...
extern int acct ( char *__name) __THROW;
...
/* Make PATH be the root directory (the starting point for absolute paths).
   This call is restricted to the super-user.  */
extern int chroot ( char *__path) __THROW __nonnull ((1)) ;

/* Prompt with PROMPT and read a string from the terminal without echoing.
   Uses /dev/tty if possible; otherwise stderr and stdin.  */
extern char *getpass ( char *__prompt) __nonnull ((1));
#endif /* Use BSD || X/Open.  */
...
/* Swab pairs bytes in the first N bytes of the area pointed to by
   ...
   is without partner.  */
extern void swab ( void * __from, void * __to,
