! testp166.f
! loop指示文とreduction節(lastmin)のテスト
...
!$xmp loop (i) on t3(:,:,i)
         ...
         ii = 1
         do i=1, N
            if(ans_val .ge. random_array(i)) then
               ii = i
         ...
         ib = 1
         ic = 1
!$xmp loop (i) on t1(i,:,:) reduction(lastmin: sa /ia/)
         do i=1, N
            if(sa .ge. a(i)) then
               ia = i
               sa = a(i)
         ...
         enddo
!$xmp loop (i) on t2(:,i,:) reduction(lastmin: sb /ib/)
         do i=1, N
            if(sb .ge. b(i)) then
               ib = i
               sb = b(i)
         ...
         enddo
!$xmp loop (i) on t3(:,:,i) reduction(lastmin: sc /ic/)
         do i=1, N
            if(sc .ge. c(i)) then
               ic = i
               sc = c(i)
