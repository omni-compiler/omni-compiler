...
#pragma xmp distribute t2(block,cyclic,cyclic) onto p
#pragma xmp distribute t3(cyclic,cyclic,block) onto p
...
#pragma xmp loop (j,i) on t1(j,i,:) reduction(lastmin:sa/ia,ja/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sa >= a[i][j]){
	ia = i;
...
#pragma xmp loop (j,i) on t2(:,j,i) reduction(lastmin:sb/ib,jb/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sb >= b[i][j]){
	ib = i;
...
#pragma xmp loop (j,i) on t3(j,:,i) reduction(lastmin:sc/ic,jc/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sc >= c[i][j]){
	ic = i;
