...
#pragma xmp loop on t1[:][i][j] reduction(lastmin:sa/ia,ja/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sa >= a[i][j]){
	ia = i;
...
#pragma xmp loop on t2[i][j][:] reduction(lastmin:sb/ib,jb/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sb >= b[i][j]){
	ib = i;
...
#pragma xmp loop on t3[i][:][j] reduction(lastmin:sc/ic,jc/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sc >= c[i][j]){
	ic = i;
