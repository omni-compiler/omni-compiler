...
#pragma xmp align b[i0][i1][i2] with ty[i2][i1][i0]
...
#pragma xmp loop (i0,i1,i2) on ty[i2][i1][i0]
  for (i2=0;i2<n;i2++){
    for (i1=0;i1<n;i1++){
      for (i0=0;i0<n;i0++){
        b[i0][i1][i2]=0;
...
#pragma xmp template tx[n][n][n]
#pragma xmp distribute tx[block][block][block] onto p
#pragma xmp align a[i][j][k] with tx[k][j][i]
#pragma xmp align b[i][j][k] with tx[k][j][i]

  int i,j,k,ierr;
  double err;

#pragma xmp loop (i,j,k) on tx[k][j][i]
  for(i=0;i<n;i++){
    for(j=0;j<n;j++){
      for(k=0;k<n;k++){
        a[i][j][k]=i+j+k+3;
