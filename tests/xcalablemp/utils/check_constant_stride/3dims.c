...
static void test(int start[DIM], int len[DIM], int stride[DIM])
{
  ...
  array[0].start  = start[0];  array[1].start  = start[1];  array[2].start  = start[2];
  array[0].length = len[0];    array[1].length = len[1];    array[2].length = len[2];
  array[0].stride = stride[0]; array[1].stride = stride[1]; array[2].stride = stride[2];
  array[0].elmts  = SIZE0;     array[1].elmts  = SIZE1;     array[2].elmts  = SIZE2;
