...
!  include 'xmp_coarray.h'
  ...
  !*** put y-axis
  if (mey>1) then
     buf2u(:,:)[mex,mey-1,mez] = p(2:mimax-1,2     ,1:mkmax)
  end if
  if (mey<ndy) then
     buf2l(:,:)[mex,mey+1,mez] = p(2:mimax-1,jmax-1,1:mkmax)
  endif
  ...
  !*** put x-axis
  if (mex>1) then
     buf1u(:,:)[mex-1,mey,mez] = p(2     ,1:mjmax,1:mkmax)
  end if
  if (mex<ndx) then
     buf1l(:,:)[mex+1,mey,mez] = p(imax-1,1:mjmax,1:mkmax)
  endif
