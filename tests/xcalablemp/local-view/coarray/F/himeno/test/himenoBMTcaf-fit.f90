...
!!  define imax, communication direction
  ...
  do i=0,ndx-1
     ...
     if(i /= 0)     mx2(i)= mx2(i) + 1     !! add left shadow
     if(i /= ndx-1) mx2(i)= mx2(i) + 1     !! add right shadow
  end do
!
  itmp= my/ndy
  my1(0)= 0
...
!!  mkmax= kmax + 1
!!!! try #1
!!  call mpi_allreduce(imax + 1, mimax, 1, mpi_integer, &
!!                     mpi_max, mpi_comm_world, ierr)
!!  call mpi_allreduce(jmax + 1, mjmax, 1, mpi_integer, &
!!                     mpi_max, mpi_comm_world, ierr)
!!  call mpi_allreduce(kmax + 1, mkmax, 1, mpi_integer, &
!!                     mpi_max, mpi_comm_world, ierr)
!!!! try #2
...
!!!!!!!!!!!!!!
  ...
  !*** put z-axis
  if (mz>1) then
     buf3u(:,:)[mx,my,mz-1] = p(2:mimax-1,2:mjmax-1,2     )
  end if
  if (mz<ndz) then
     buf3l(:,:)[mx,my,mz+1] = p(2:mimax-1,2:mjmax-1,kmax-1)
  endif
...
!!!!!!!!!!!!!!
  ...
  !*** put y-axis
  if (my>1) then
     buf2u(:,:)[mx,my-1,mz] = p(2:mimax-1,2     ,1:mkmax)
  end if
  if (my<ndy) then
     buf2l(:,:)[mx,my+1,mz] = p(2:mimax-1,jmax-1,1:mkmax)
  endif
  ...
  !*** unpack y-axis
  if (my<ndy) then
     p(2:mimax-1,jmax,1:mkmax) = buf2u(:,:)
  end if
  if (my>1) then
     p(2:mimax-1,1   ,1:mkmax) = buf2l(:,:)
  endif
...
!!!!!!!!!!!!!!
  ...
  !*** put x-axis
  if (mx>1) then
     buf1u(:,:)[mx-1,my,mz] = p(2     ,1:mjmax,1:mkmax)
  end if
  if (mx<ndx) then
     buf1l(:,:)[mx+1,my,mz] = p(imax-1,1:mjmax,1:mkmax)
  endif
  ...
  !*** unpack x-axis
  if (mx<ndx) then
     p(imax,1:mjmax,1:mkmax) = buf1u(:,:)
  end if
  if (mx>1) then
     p(1   ,1:mjmax,1:mkmax) = buf1l(:,:)
  endif
