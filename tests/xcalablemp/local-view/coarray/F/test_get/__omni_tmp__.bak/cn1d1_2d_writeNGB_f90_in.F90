...
# 17 "xmp_lib.h"
 INTEGER :: this_image
 EXTERNAL this_image
# 7 "cn1d1-writeNGB.f90"
 CHARACTER ( LEN= 200 ) :: wbuf1 ( 1 : 1 )

# 5 "cn1d1-writeNGB.f90"
 INTEGER :: xmp_node_num
 EXTERNAL xmp_node_num
# 6 "cn1d1-writeNGB.f90"
 INTEGER :: nerr
 INTEGER :: xmp_size_array ( 0 : 15 , 0 : 6 )
# 7 "cn1d1-writeNGB.f90"
 CHARACTER ( LEN= 200 ) :: wbuf2 ( 1 : 1 )
...
# 4 "cn1d1-writeNGB.f90"
 INTEGER ( KIND= 2 ) :: a ( 1 : 100 )
 INTEGER :: i
...
# 17 "xmp_lib.h"
 ...
 INTEGER ( KIND= 8 ) :: xmpf_descptr_wbuf2
# 4 "cn1d1-writeNGB.f90"
 INTEGER ( KIND= 2 ) :: a_org ( 1 : 100 )
...
# 4 "xmp_lib.h"
 TYPE :: xmp_desc
  ...
  INTEGER ( KIND= 8 ) :: desc
 END TYPE xmp_desc
 POINTER ( xmpf_crayptr_wbuf2 , wbuf2 )
 POINTER ( xmpf_crayptr_a , a )
...
# 1763 "xmp_lib_coarray_get.h"
 ...
 END INTERFACE
 COMMON / xmpf_descptr_xmpf_main / xmpf_descptr_a , xmpf_descptr_wbuf2
 COMMON / xmpf_crayptr_xmpf_main / xmpf_crayptr_a , xmpf_crayptr_wbuf2
 COMMON / XMP_COMMON / xmp_size_array

 CALL xmpf_nodes_alloc_ ( XMP_DESC_p , 1 )
 CALL xmpf_nodes_dim_size_ ( XMP_DESC_p , 0 , (-1) )
 CALL xmpf_nodes_init_GLOBAL_ ( XMP_DESC_p )
# 9 "cn1d1-writeNGB.f90"
 me = xmp_node_num ( )
 DO i = 1 , 100 , 1
# 17 "cn1d1-writeNGB.f90"
  a_org ( i ) = i + me * 100
# 18 "cn1d1-writeNGB.f90"
  a ( i ) = a_org ( i )
 END DO
# 21 "cn1d1-writeNGB.f90"
 CALL xmpf_sync_all ( )
# 24 "cn1d1-writeNGB.f90"
 IF ( me == 1 ) THEN
  WRITE ( unit = wbuf1 , fmt = * ) xmpf_coarray_get1d ( xmpf_descptr_a , a ( 15 ) , 2 , xmpf_coarray_get_image_index (&
   xmpf_descptr_a , 1 , 2 ) , 1 , a ( 22 ) , 13 )
 END IF
# 28 "cn1d1-writeNGB.f90"
 IF ( me == 2 ) THEN
  WRITE ( unit = wbuf2 , fmt = * ) a ( 15 : 100 : 7 )
 END IF
# 33 "cn1d1-writeNGB.f90"
 CALL xmpf_sync_all ( )
# 36 "cn1d1-writeNGB.f90"
 nerr = 0
# 37 "cn1d1-writeNGB.f90"
 IF ( me == 1 ) THEN
# 38 "cn1d1-writeNGB.f90"
  tmp = xmpf_coarray_get0d ( xmpf_descptr_wbuf2 , wbuf2 ( 1 ) , 200 , xmpf_coarray_get_image_index ( xmpf_descptr_wbuf2 , 1 , 2 )&
   , 0 )
# 39 "cn1d1-writeNGB.f90"
  IF ( wbuf1 ( 1 ) /= tmp ) THEN
   WRITE ( unit = * , fmt = * )"[1] wbuf1 and wbuf2[2] should be the same."
# 41 "cn1d1-writeNGB.f90"
   nerr = nerr + 1
  END IF
 END IF
# 45 "cn1d1-writeNGB.f90"
 IF ( nerr == 0 ) THEN
# 46 "cn1d1-writeNGB.f90"
  PRINT '("result[",i0,"] OK")' , me
 ELSE
# 48 "cn1d1-writeNGB.f90"
...
SUBROUTINE xmpf_traverse_coarraysize_1xmpf_main ( )
 ...
 CALL xmpf_coarray_count_size ( 100 , 2 )
 CALL xmpf_coarray_count_size ( 1 , 200 )
END SUBROUTINE xmpf_traverse_coarraysize_1xmpf_main

SUBROUTINE xmpf_traverse_initcoarray_1xmpf_main ( )
 INTEGER ( KIND= 8 ) :: xmpf_descptr_a
 INTEGER ( KIND= 8 ) :: xmpf_crayptr_wbuf2
 EXTERNAL xmpf_coarray_share_pool
 INTEGER ( KIND= 8 ) :: xmpf_descptr_wbuf2
 INTEGER ( KIND= 8 ) :: xmpf_crayptr_a
 EXTERNAL xmpf_coarray_set_coshape
 COMMON / xmpf_descptr_xmpf_main / xmpf_descptr_a
 COMMON / xmpf_crayptr_xmpf_main / xmpf_crayptr_a
 COMMON / xmpf_descptr_xmpf_main / xmpf_descptr_wbuf2
 COMMON / xmpf_crayptr_xmpf_main / xmpf_crayptr_wbuf2

 CALL xmpf_coarray_share_pool ( xmpf_descptr_a , xmpf_crayptr_a , 100 , 2 ,"a" , 1 )
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_a , 1 , 1 )
 CALL xmpf_coarray_share_pool ( xmpf_descptr_wbuf2 , xmpf_crayptr_wbuf2 , 1 , 200 ,"wbuf2" , 5 )
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_wbuf2 , 1 , 1 )
