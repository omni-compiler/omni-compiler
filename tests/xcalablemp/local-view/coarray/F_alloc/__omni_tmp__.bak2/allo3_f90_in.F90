...
# 6 "allo3.f90"
 REAL , ALLOCATABLE :: b ( : )
 EXTERNAL xmpf_traverse_initcoarray_allo3
 POINTER ( xmpf_crayptr_b2 , b2 )
 EXTERNAL xmpf_traverse_countcoarray_allo3
 POINTER ( xmpf_crayptr_b1 , b1 )
 INTEGER :: xmpf_coarray_get_image_index
 EXTERNAL xmpf_coarray_get_image_index
 INTEGER ( KIND= 8 ) :: xmpf_descptr_b1
# 17 "xmp_lib.h"
 INTEGER :: num_images
 EXTERNAL num_images
# 5 "allo3.f90"
...
# 1347 "xmp_lib_coarray_alloc.h"
 ...
 END INTERFACE
 COMMON / xmpf_descptr_allo3 / xmpf_descptr_b1 , xmpf_descptr_b2
 COMMON / xmpf_crayptr_allo3 / xmpf_crayptr_b1 , xmpf_crayptr_b2
 COMMON / XMP_COMMON / xmp_size_array

 CALL xmpf_coarray_proc_init ( xmpf_resource_tag )
 WRITE ( unit = * , fmt = * )"allocated(a1),allocated(c1):" , associated ( a1 ) , allocated ( c1 )
 WRITE ( unit = * , fmt = * )"allocated(a2),allocated(c2):" , associated ( a2 ) , allocated ( c2 )
 CALL xmpf_coarray_put_scalar ( xmpf_descptr_b2 , b2 ( 3 ) , 4 , xmpf_coarray_get_image_index ( xmpf_descptr_b2 , 2 , 2 , 1 ) ,&
  1.234 , 1 , 0 )
# 13 "allo3.f90"
 CALL xmpf_coarray_alloc2d ( xmpf_descptr_a1 , a1 , 2 * 3 , 4 , xmpf_resource_tag , 2 , 1 , 2 , 1 , 3 )
# 13 "allo3.f90"
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_a1 , 1 , 1 )
# 13 "allo3.f90"
 CALL xmpf_coarray_alloc1d ( xmpf_descptr_a2 , a2 , 8 , 4 , xmpf_resource_tag , 1 , 1 , 8 )
# 13 "allo3.f90"
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_a2 , 2 , 1 , 4 , 1 )
# 14 "allo3.f90"
 ALLOCATE ( c1 ( 2 , 3 ) , c2 ( 8 ) )
 WRITE ( unit = * , fmt = * )"allocate (a1(2,3)[*],a2(8)[4,*])"
 WRITE ( unit = * , fmt = * )"allocate (c1(2,3),c2(8))"
 WRITE ( unit = * , fmt = * )"allocated(a1),allocated(c1):" , associated ( a1 ) , allocated ( c1 )
 WRITE ( unit = * , fmt = * )"allocated(a2),allocated(c2):" , associated ( a2 ) , allocated ( c2 )
 WRITE ( unit = * , fmt = * )"size(a1),size(c1):" , size ( a1 ) , size ( c1 )
 WRITE ( unit = * , fmt = * )"size(a2),size(c2):" , size ( a2 ) , size ( c2 )
 CALL xmpf_coarray_put_scalar ( xmpf_descptr_a2 , a2 ( 3 ) , 4 , xmpf_coarray_get_image_index ( xmpf_descptr_a2 , 2 , 2 , 1 ) ,&
  1.234 , 1 , 0 )
# 24 "allo3.f90"
 c2 ( 3 ) = 1.234
# 26 "allo3.f90"
 CALL xmpf_coarray_dealloc2d ( xmpf_descptr_a1 , a1 , xmpf_resource_tag )
# 26 "allo3.f90"
 CALL xmpf_coarray_dealloc1d ( xmpf_descptr_a2 , a2 , xmpf_resource_tag )
# 27 "allo3.f90"
 DEALLOCATE ( c1 , c2 )
 WRITE ( unit = * , fmt = * )"deallocate (a1,a2)"
 WRITE ( unit = * , fmt = * )"deallocate (c1,c2)"
 WRITE ( unit = * , fmt = * )"allocated(a1),allocated(c1):" , associated ( a1 ) , allocated ( c1 )
 WRITE ( unit = * , fmt = * )"allocated(a2),allocated(c2):" , associated ( a2 ) , allocated ( c2 )
# 33 "allo3.f90"
 CALL xmpf_coarray_alloc1d ( xmpf_descptr_a2 , a2 , 8 , 4 , xmpf_resource_tag , 1 , 3 , 12 )
# 33 "allo3.f90"
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_a2 , 2 , 1 , 2 , 1 )
# 34 "allo3.f90"
 ALLOCATE ( c2 ( 3 : 12 ) )
 WRITE ( unit = * , fmt = * )"allocate (a2(3:12)[2,*])"
 WRITE ( unit = * , fmt = * )"allocate (c2(3:12))"
 WRITE ( unit = * , fmt = * )"allocated(a1),allocated(c1):" , associated ( a1 ) , allocated ( c1 )
 WRITE ( unit = * , fmt = * )"allocated(a2),allocated(c2):" , associated ( a2 ) , allocated ( c2 )
 WRITE ( unit = * , fmt = * )"size(a2),size(c2):" , size ( a2 ) , size ( c2 )
...
SUBROUTINE xmpf_traverse_countcoarray_allo3 ( )
 EXTERNAL xmpf_coarray_count_size

 CALL xmpf_coarray_count_size ( 6 , 4 )
 CALL xmpf_coarray_count_size ( 8 , 4 )
END SUBROUTINE xmpf_traverse_countcoarray_allo3

SUBROUTINE xmpf_traverse_initcoarray_allo3 ( )
 INTEGER ( KIND= 8 ) :: xmpf_descptr_b1
 EXTERNAL xmpf_coarray_share_pool
 INTEGER ( KIND= 8 ) :: xmpf_descptr_b2
 INTEGER ( KIND= 8 ) :: xmpf_crayptr_b2
 EXTERNAL xmpf_coarray_set_coshape
 INTEGER ( KIND= 8 ) :: xmpf_crayptr_b1
 COMMON / xmpf_descptr_allo3 / xmpf_descptr_b1
 COMMON / xmpf_crayptr_allo3 / xmpf_crayptr_b1
 COMMON / xmpf_descptr_allo3 / xmpf_descptr_b2
 COMMON / xmpf_crayptr_allo3 / xmpf_crayptr_b2

 CALL xmpf_coarray_share_pool ( xmpf_descptr_b1 , xmpf_crayptr_b1 , 6 , 4 ,"b1" , 2 )
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_b1 , 1 , 1 )
 CALL xmpf_coarray_share_pool ( xmpf_descptr_b2 , xmpf_crayptr_b2 , 8 , 4 ,"b2" , 2 )
 CALL xmpf_coarray_set_coshape ( xmpf_descptr_b2 , 2 , 1 , 4 , 1 )
END SUBROUTINE xmpf_traverse_initcoarray_allo3
