...
#define SIZE 6
#define DIMS 5
int a[SIZE][SIZE][SIZE][SIZE][SIZE], b[SIZE][SIZE][SIZE][SIZE][SIZE], ret = 0;
			...
			for(stride[3]=1;stride[3]<4;stride[3]++){
			  for(start[4]=0;start[4]<2;start[4]++){
				...
				if(start[0]+(len[0]-1)*stride[0] < SIZE && start[1]+(len[1]-1)*stride[1] < SIZE
				  ...
				  if(xmp_node_num() == 1){
				    a[start[0]:len[0]:stride[0]][start[1]:len[1]:stride[1]][start[2]:len[2]:stride[2]][start[3]:len[3]:stride[3]][start[4]:len[4]:stride[4]]:[2] 
				      = b[start[0]:len[0]:stride[0]][start[1]:len[1]:stride[1]][start[2]:len[2]:stride[2]][start[3]:len[3]:stride[3]][start[4]:len[4]:stride[4]];
				  }
					...
					for(int k=0;k<len[2];k++){
					  ...
					  for(int m=0;m<len[3];m++){
					    ...
					    for(int n=0;n<len[4];n++){
					      ...
					      if(a[position0][position1][position2][position3][position4] != 1){
