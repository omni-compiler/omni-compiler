! testp177.f
! loop指示文とreduction節(firstmax)のテスト
...
!$xmp distribute t3(cylic,block,block) onto p
      integer a(N,N), sa
      real*8  b(N,N), sb
      real*4  c(N,N), sc
      integer ia, ib, ic, ii
...
!$xmp loop (i,j) on t3(i,:,j)
         ...
         do j=1, N
            do i=1, N
               m = (j-1)*N+i
               if(ans_val .lt. random_array(m)) then
                  ii = i
                  jj = j
         ...
         jb = 1
         jc = 1
!$xmp loop (i,j) on t1(i,j,:) reduction(firstmax: sa /ia, ja/)
         do j=1, N
            do i=1, N
               if(sa .lt. a(i,j)) then
                  ia = i
            ...
            enddo
         enddo
!$xmp loop (i,j) on t2(:,i,j) reduction(firstmax: sb /ib, jb/)
         do j=1, N
            do i=1, N
               if(sb .lt. b(i,j)) then
                  ib = i
            ...
            enddo
         enddo
!$xmp loop (i,j) on t3(i,:,j) reduction(firstmax: sc /ic, jc/)
         do j=1, N
            do i=1, N
               if(sc .lt. c(i,j)) then
                  ic = i
      ...
      print *, xmp_node_num(), 'testp177.f ', result
