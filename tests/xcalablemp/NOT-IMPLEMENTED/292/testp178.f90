...
!$xmp loop (i,j) on t3(i,:,j)
         ...
         do j=1, N
            do i=1, N
               m = (j-1)*N+i
               if(ans_val .gt. random_array(m)) then
                  ii = i
                  jj = j
         ...
         jb = 1
         jc = 1
!$xmp loop (i,j) on t1(i,j,:) reduction(firstmin: sa /ia, ja/)
         do j=1, N
            do i=1, N
               if(sa .gt. a(i,j)) then
                  ia = i
            ...
            enddo
         enddo
!$xmp loop (i,j) on t2(:,i,j) reduction(firstmin: sb /ib, jb/)
         do j=1, N
            do i=1, N
               if(sb .gt. b(i,j)) then
                  ib = i
            ...
            enddo
         enddo
!$xmp loop (i,j) on t3(i,:,j) reduction(firstmin: sc /ic, jc/)
         do j=1, N
            do i=1, N
               if(sc .gt. c(i,j)) then
                  ic = i
      ...
      print *, xmp_node_num(), 'testp178.f ', result
