...
#define SIZE 3
#define DIMS 7
int a[SIZE][SIZE][SIZE][SIZE][SIZE][SIZE][SIZE], b[SIZE][SIZE][SIZE][SIZE][SIZE][SIZE][SIZE], ret = 0;
				...
				for(start[5]=0;start[5]<2;start[5]++){
				  for(len[5]=1;len[5]<=SIZE;len[5]++){
				    for(stride[5]=1;stride[5]<4;stride[5]++){
				      for(start[6]=0;start[6]<2;start[6]++){
					for(len[6]=1;len[6]<=SIZE;len[6]++){
					  for(stride[6]=1;stride[6]<4;stride[6]++){
						...
						for(int j=0;j<SIZE;j++)
					      ...
					      if(xmp_node_num() == 1){
						a[start[0]:len[0]:stride[0]][start[1]:len[1]:stride[1]][start[2]:len[2]:stride[2]][start[3]:len[3]:stride[3]][start[4]:len[4]:stride[4]][start[5]:len[5]:stride[5]][start[6]:len[6]:stride[6]]
						  = b[start[0]:len[0]:stride[0]][start[1]:len[1]:stride[1]][start[2]:len[2]:stride[2]][start[3]:len[3]:stride[3]][start[4]:len[4]:stride[4]][start[5]:len[5]:stride[5]][start[6]:len[6]:stride[6]]:[2];
						  ...
						  for(int j=0;j<len[1];j++){
							...
							for(int n=0;n<len[4];n++){
							  ...
							  for(int p=0;p<len[5];p++){
							    ...
							    for(int q=0;q<len[6];q++){
							      int position6 = start[6]+q*stride[6];
							      if(a[position0][position1][position2][position3][position4][position5][position6] != 2){
								fprintf(stderr, "a[%d:%d:%d][%d:%d:%d][%d:%d:%d][%d:%d:%d][%d:%d:%d][%d:%d:%d][%d:%d:%d] ", 
