...
int main(){
  ...
  int i0,i1,i2,i3,i4,myrank,ierr;
  double b[N][N][N][N][N],err;
...
#pragma xmp loop (i0) on tx(*,*,*,*,i0)
          for(i0=0;i0<n;i0++){
            a[i4][i3][i2][i1][i0]=i4+i3+i2+i0+i1+1;
          }
        }
      }
    }
  ...
  for(i4=1;i4<n;i4++){
    for(i3=1;i3<n;i3++){
      for(i2=1;i2<n;i2++){
        for(i1=1;i1<n;i1++){
          for(i0=1;i0<n;i0++){
            err=err+fabs(b[i4][i3][i2][i1][i0]-i4-i3-i2-i0-i1-1);
          }
        }
      }
    }
