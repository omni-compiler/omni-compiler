...
#pragma xmp template tx[n]
#pragma xmp template ty[n][n]
#pragma xmp distribute tx[block] onto p
#pragma xmp distribute ty[gblock(m1)][gblock(m1)] onto q
#pragma xmp align a[i0][*] with tx[i0]
#pragma xmp align b[i0][i1] with ty[i1][i0]
...
#pragma xmp loop (i0,i1) on ty[i1][i0]
  for(i0=0;i0<n;i0++){
    for(i1=0;i1<n;i1++){
      b[i0][i1]=0;
    }
