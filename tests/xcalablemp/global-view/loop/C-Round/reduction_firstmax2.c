...
#pragma xmp distribute t3(cyclic,block,block) onto p
int a[N][N],sa=0;
double b[N][N],sb=0.0;
float c[N][N],sc=0.0;
int ia=0,ib=0,ic=0,ii=0,ja=0,jb=0,jc=0,jj=0;
...
#pragma xmp loop (j,i) on t1(j,i,:) reduction(firstmax:sa/ia,ja/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sa < a[i][j]){
	ia = i;
...
#pragma xmp loop (j,i) on t2(:,j,i) reduction(firstmax:sb/ib,jb/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sb < b[i][j]){
	ib = i;
...
#pragma xmp loop (j,i) on t3(j,:,i) reduction(firstmax:sc/ic,jc/)
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(sc < c[i][j]){
	ic = i;
