...
double b[N],sb=0.0;
float c[N],sc=0.0;
int ia=1,ib=1,ic=1,ii=0,i,k,result=0,ans_val=0;
...
#pragma xmp loop (i) on t1(i,:,:) reduction(lastmax:sa/ia/)
  for(i=0;i<N;i++){
    if(sa <= a[i]){
      ia = i;
      sa = a[i];
...
#pragma xmp loop (i) on t2(:,i,:) reduction(lastmax:sb/ib/)
  for(i=0;i<N;i++){
    if(sb <= b[i]){
      ib = i;
      sb = b[i];
...
#pragma xmp loop (i) on t3(:,:,i) reduction(lastmax:sc/ic/)
  for(i=0;i<N;i++){
    if(sc <= c[i]){
      ic = i;
      sc = c[i];
