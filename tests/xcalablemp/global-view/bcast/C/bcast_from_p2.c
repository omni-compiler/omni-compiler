...
#pragma xmp bcast (a) from p(j,k) 
#pragma xmp bcast (b) from p(j,k) 
#pragma xmp bcast (c) from p(j,k) 
#pragma xmp bcast (aa) from p(j,k)
#pragma xmp bcast (bb) from p(j,k) 
#pragma xmp bcast (cc) from p(j,k) 
      ans = (k-1)*4+j;
      if(a != ans)         result = -1;
