...
#define POSTWAIT(rank, tag) MPI_Sendrecv(&post_tag, 1, MPI_INT, (rank), (tag), &wait_tag, 1, MPI_INT, (rank), (tag), MPI_COMM_WORLD, MPI_STATUS_IGNORE)
#define ALIGN_ELMTS (64/8)
#define ALIGN(s) ((((s)-1)/ALIGN_ELMTS + 1) * ALIGN_ELMTS)
MPI_Win win_host, win_dev;
double *red_temp;
int q_offset;
int r_offset;
int w_offset;
...
//      >           rcond=1.0d-1 )
  ...
  double *restrict v, *restrict aelt, *restrict a, *restrict x, *restrict z, *restrict p, *restrict q, *restrict r, *restrict w;
...
#ifdef USE_RMA
  int const vec_buf_len  = ALIGN(na/num_proc_rows+2);
  int const norm_buf_len = ALIGN(2);
  int const red_buf_len  = ALIGN(1);
  int const rma_buf_len  = vec_buf_len*3 + norm_buf_len*2 + red_buf_len;
  double *rma_buf = alloc_mem(sizeof(double), rma_buf_len);
  q_offset = 0;
  r_offset = vec_buf_len;
  w_offset = r_offset + vec_buf_len;
  red_temp_offset = w_offset + vec_buf_len;
  int const norm_temp1_offset = red_temp_offset + red_buf_len;
  int const norm_temp2_offset = norm_temp1_offset + norm_buf_len;
  q = rma_buf + q_offset;
  r = rma_buf + r_offset;
  w = rma_buf + w_offset;
  red_temp = w + vec_buf_len;
#else
  q = alloc_mem(sizeof(double), na/num_proc_rows+2);
  r = alloc_mem(sizeof(double), na/num_proc_rows+2);
  w = alloc_mem(sizeof(double), na/num_proc_rows+2);
#endif
  ...
  double rnorm;
#ifdef USE_RMA
  double *norm_temp1 = rma_buf + norm_temp1_offset, *norm_temp2 = rma_buf + norm_temp2_offset;
#else
...
#ifdef USE_RMA
#pragma acc data pcopyin(colidx[0:nz], rowstr[0:na+1], x[0:ROW_ARRAY_LEN], z[0:ROW_ARRAY_LEN], a[0:nz], p[0:ROW_ARRAY_LEN], rma_buf[0:rma_buf_len]) pcopyin(reduce_recv_lengths[0:num_proc_cols])
#else
#pragma acc data pcopyin(colidx[0:nz], rowstr[0:na+1], x[0:ROW_ARRAY_LEN], z[0:ROW_ARRAY_LEN], a[0:nz], p[0:ROW_ARRAY_LEN], q[0:ROW_ARRAY_LEN], r[0:ROW_ARRAY_LEN], w[0:ROW_ARRAY_LEN]) pcopyin(reduce_recv_lengths[0:num_proc_cols])
  ...
  {
#ifdef USE_RMA
  MPI_Win_create((void*)rma_buf, rma_buf_len*sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_host);
#pragma acc host_data use_device(rma_buf)
  MPI_Win_create((void*)rma_buf, rma_buf_len*sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_dev);
  MPI_Win_lock_all(0, win_host);
  MPI_Win_lock_all(0, win_dev);
#endif
//--------------------------------------------------------------------
//---->
...
#ifdef USE_RMA
      MPI_Win_sync(win_host);
      POSTWAIT(reduce_exch_proc[i], 0);
      MPI_Win_sync(win_host);
      ierr = MPI_Put(norm_temp1, 2, dp_type, reduce_exch_proc[i], norm_temp2_offset, 2, dp_type, win_host);
      MPI_Win_flush_all(win_host);
      MPI_Win_sync(win_host);
      POSTWAIT(reduce_exch_proc[i], 0);
      MPI_Win_sync(win_host);
#else
      ierr = MPI_Irecv(norm_temp2,
                       2,
                       dp_type,
                       reduce_exch_proc[i],
                       i+1,
...
#ifdef USE_RMA
  MPI_Win_unlock_all(win_host);
  MPI_Win_unlock_all(win_dev);
  MPI_Win_free(&win_host);
  MPI_Win_free(&win_dev);
#endif
  }//end of acc data
...
//--------------------------------------------------------------------
  t = timer_read(t_total);

  ierr = MPI_Reduce(&t, &tmax, 1, dp_type, MPI_MAX, root, MPI_COMM_WORLD);
  ...
  ierr = MPI_Reduce(t1, tsum,  t_last+2, dp_type, MPI_SUM, 0, MPI_COMM_WORLD);
  ierr = MPI_Reduce(t1, tming, t_last+2, dp_type, MPI_MIN, 0, MPI_COMM_WORLD);
  ierr = MPI_Reduce(t1, tmaxg, t_last+2, dp_type, MPI_MAX, 0, MPI_COMM_WORLD);
...
void initialize_mpi()
{
  ...
  ierr = MPI_Bcast(&(timeron), 1, MPI_LOGICAL, 0, MPI_COMM_WORLD);
...
//--------------------------------------------------------------------
  for(int i = 0; i < l2npcols; i++){
    ...
    POSTWAIT(reduce_exch_proc[i], 0);
    MPI_Win_sync(win_host);
    ierr = MPI_Put(&rho, 1, dp_type, reduce_exch_proc[i], red_temp_offset, 1, dp_type, win_host);
    ierr = MPI_Win_flush_all(win_host);
    MPI_Win_sync(win_host);
    POSTWAIT(reduce_exch_proc[i], 0);
    MPI_Win_sync(win_host);
    tmp = red_temp[0];
...
#ifdef _USE_ALLREDUCE
    if (timeron) timer_start(t_rcomm);
    MPI_Allreduce(MPI_IN_PLACE, w, ROW_ARRAY_LEN, MPI_DOUBLE, MPI_SUM, row_comm);
    if (timeron) timer_stop(t_rcomm);
#else
    for(int i = l2npcols - 1; i >= 0; i--){
      if (timeron) timer_start(t_rcomm);
#ifdef USE_RMA
      MPI_Win_sync(win_dev);
      POSTWAIT(reduce_exch_proc[i], 0);
      MPI_Win_sync(win_dev);
      int reduce_start = reduce_recv_starts[i];
      int reduce_length = reduce_recv_lengths[i];
      ierr = MPI_Get(&q[reduce_start], reduce_length, dp_type, reduce_exch_proc[i], w_offset + reduce_start, reduce_length, dp_type, win_dev);
      ierr = MPI_Win_flush_all(win_dev);
      MPI_Win_sync(win_dev);
      POSTWAIT(reduce_exch_proc[i], 0);
      MPI_Win_sync(win_dev);
#else
      ierr = MPI_Irecv(&q[reduce_recv_starts[i]],
                       ...
                       MPI_COMM_WORLD,
                       &request);
      ierr = MPI_Send(&w[reduce_send_starts[i]],
...
#ifdef USE_RMA
      MPI_Win_sync(win_dev);
      POSTWAIT(exch_proc, 0);
      MPI_Win_sync(win_dev);
      ierr = MPI_Get(q, send_len, dp_type, exch_proc, w_offset + send_start, send_len, dp_type, win_dev);
      ierr = MPI_Win_flush_all(win_dev);
      MPI_Win_sync(win_dev);
      POSTWAIT(exch_proc, 0);
      MPI_Win_sync(win_dev);
...
//--------------------------------------------------------------------
    for(int i = 0; i < l2npcols; i++){
      ...
      POSTWAIT(reduce_exch_proc[i], 0);
      MPI_Win_sync(win_host);
      ierr = MPI_Put(&d, 1, dp_type, reduce_exch_proc[i], red_temp_offset, 1, dp_type, win_host);
      ierr = MPI_Win_flush_all(win_host);
      MPI_Win_sync(win_host);
      POSTWAIT(reduce_exch_proc[i], 0);
      MPI_Win_sync(win_host);
...
//--------------------------------------------------------------------
  ...
  for(int i = l2npcols - 1; i >= 0; i--){
    ...
    int reduce_start = reduce_recv_starts[i];
    int reduce_length = reduce_recv_lengths[i];
    ierr = MPI_Get(&r[reduce_start], reduce_length, dp_type, reduce_exch_proc[i], w_offset + reduce_start, reduce_length, dp_type, win_dev);
    ierr = MPI_Win_flush_all(win_dev);
    MPI_Win_sync(win_dev);
    POSTWAIT(reduce_exch_proc[i], 0);
    MPI_Win_sync(win_dev);
#else
    ierr = MPI_Irecv(&r[reduce_recv_starts[i]],
                     reduce_recv_lengths[i],
                     dp_type,
                     reduce_exch_proc[i],
                     i+1,
...
//--------------------------------------------------------------------
  if( l2npcols != 0 ){
    ...
    POSTWAIT(exch_proc, 0);
    MPI_Win_sync(win_dev);
    ierr = MPI_Get(r, send_len, dp_type, exch_proc, w_offset + send_start, send_len, dp_type, win_dev);
    ierr = MPI_Win_flush_all(win_dev);
    MPI_Win_sync(win_dev);
    POSTWAIT(exch_proc, 0);
    MPI_Win_sync(win_dev);
...
//--------------------------------------------------------------------
  for(int i = 0; i < l2npcols; i++){
    ...
    POSTWAIT(reduce_exch_proc[i], 0);
    MPI_Win_sync(win_host);
    ierr = MPI_Put(&sum, 1, dp_type, reduce_exch_proc[i], red_temp_offset, 1, dp_type, win_host);
    ierr = MPI_Win_flush_all(win_host);
    MPI_Win_sync(win_host);
    POSTWAIT(reduce_exch_proc[i], 0);
    MPI_Win_sync(win_host);
