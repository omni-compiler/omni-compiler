...
!....
      if( xmp_all_num_nodes() .ne. lnpy*lnpz ) then
          write(*,*) 'ERROR: XANN must match with lnpy*lnpz.', &
                                  xmp_all_num_nodes(), lnpy, lnpz
          go to 999
      end if 
!....
...
!....
      wcheck = 0.0d0
! !$XMP LOOP (iy,iz) ON t(*,iy,iz) REDUCTION(+:wcheck)
...
! !$XMP TASK ON proc(1)
      if( xmp_node_num() .eq. 1 ) then
        write(*,*) '--------------------------------------------------'
        write(*,*) 'xmp2rh, lnpy, lnpz = ', lnpy, lnpz
