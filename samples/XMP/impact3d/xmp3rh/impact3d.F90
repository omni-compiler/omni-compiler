...
!....
      if( xmp_all_num_nodes() .ne. lnpx*lnpy*lnpz ) then
          write(*,*) 'ERROR: XANN must match with lnpx*lnpy*lnpz.', &
                                xmp_all_num_nodes(), lnpx, lnpy, lnpz
          go to 999
      end if 
!....
...
!....
      wcheck = 0.0d0
!$XMP LOOP (ix,iy,iz) ON t(ix,iy,iz) REDUCTION(+:wcheck)
      do iz = lz/4 + 1, lz/4 * 3
      do iy = ly/4 + 1, ly/4 * 3
      do ix = lx/4 + 1, lx/4 * 3
...
! !$XMP TASK ON proc(1)
      if( xmp_node_num() .eq. 1 ) then
        write(*,*) '--------------------------------------------------'
        write(*,*) 'xmp3rh, lnpx, lnpy, lnpz = ', lnpx, lnpy, lnpz
