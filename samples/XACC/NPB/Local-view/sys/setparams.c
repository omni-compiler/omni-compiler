...
void write_cg_info(FILE *fp, int nprocs, char class) 
{
  ...
  */
  fprintf( fp, "#define na (%d)\n", na);  //  fprintf( fp, "const int na = %d;\n", na);
  fprintf( fp, "const int nonzer = %d;\n", nonzer);
  fprintf( fp, "const int niter = %d;\n", niter);
  fprintf( fp, "const double shift = %s;\n", shift);
  fprintf( fp, "const double rcond = %s;\n", rcond);
  ...
  */
  ...
  fprintf( fp, "const int nnodes_compiled = %d;\n", nprocs );
  fprintf( fp, "#define num_proc_cols (%d)\n", num_proc_cols); //fprintf( fp, "const int num_proc_cols = %d;\n", num_proc_cols );
  fprintf( fp, "#define num_proc_rows (%d)\n", num_proc_rows); //fprintf( fp, "const int num_proc_rows = %d;\n", num_proc_rows );
}
...
setparams: File %s doesn't exist. To build the NAS benchmarks\n\
  ...
  switch(type) {
	...
	/*
          ...
          put_def_string(fp, "cs1", mpif77);
          put_def_string(fp, "cs2", flink);
          put_def_string(fp, "cs3", fmpi_lib);
          put_def_string(fp, "cs4", fmpi_inc);
          put_def_string(fp, "cs5", fflags);
          put_def_string(fp, "cs6", flinkflags);
	  put_def_string(fp, "cs7", randfile);
	*/
	/*
	fprintf(fp, "char compiletime[] = \"%s\";\n", compiletime);
	fprintf(fp, "char npbversion[] = \"%s\";\n", VERSION);
	fprintf(fp, "char cs1[] = \"%s\";\n", mpicc);
	fprintf(fp, "char cs2[] = \"%s\";\n", cflags);
	fprintf(fp, "char cs3[] = \"%s\";\n", clink);
	fprintf(fp, "char cs4[] = \"%s\";\n", clinkflags);
	fprintf(fp, "char cs5[] = \"%s\";\n", cmpi_lib);
	fprintf(fp, "char cs6[] = \"%s\";\n", cmpi_inc);
	fprintf(fp, "char cs7[] = \"%s\";\n", randfile);
	break;
	*/
      case IS:
      case DT:
...
void write_convertdouble_info(int type, FILE *fp)
{
  ...
  case CG:
    ...
    //    fprintf(fp, "const int convertdouble = 1;\n");
    fprintf(fp, "#define convertdouble 1\n");
#else
    //    fprintf(fp, "%sparameter (convertdouble = .false.)\n", FINDENT);
    //    fprintf(fp, "const int convertdouble = 0;\n");
    fprintf(fp, "#define convertdouble 0\n");
#endif
    break;
  }
}
