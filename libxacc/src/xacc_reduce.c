...
#define REDUCE(left, right, op) \
{\
  switch(op){		 \
  case _XMP_N_REDUCE_SUM: \
    *(left) += *(right); break;\
  default:\
    _XMP_fatal("unknown operation for reduction"); \
  }\
}

/* void _XACC_reduce_NODES_ENTIRE(_XMP_nodes_t *nodes, void *addr, int count, int datatype, int op);*/
/* void _XACC_reduce_gpu_FLMM_NODES_ENTIRE(_XMP_nodes_t *nodes, void *addr, int count, int datatype, int op, int num_locs, ...); */
/* void _XACC_reduce_gpu_CLAUSE(void *data_addr, int count, int datatype, int op); */
/* void _XACC_reduce_gpu_FLMM_CLAUSE(void *data_addr, int count, int datatype, int op, int num_locs, ...); */

static void reduce(void *left, void *right, int datatype, int op)
{
  switch(datatype){
  case _XMP_N_TYPE_INT:
    REDUCE((int*)left, (int*)right, op); break;
  case _XMP_N_TYPE_DOUBLE:
    REDUCE((double*)left, (double*)right, op); break;
  case _XMP_N_TYPE_FLOAT:
    REDUCE((float*)left, (float*)right, op); break;
  default:
...
static void cudaErrorCheck(cudaError_t e)
{
 if(e != cudaSuccess){
...
void _XACC_reduce_NODES_ENTIRE(_XMP_nodes_t *nodes, _XACC_arrays_t *arrays_desc, int count, int datatype, int op)
{
  if (count == 0) {
  ...
  void *host_buf = _XMP_alloc(size * num_devices);
  cudaError_t e;

  // copy dev to host
  for(int dev = device->lb; dev <= device->ub; dev += device->step){
    _XACC_array_t *array_desc = &(arrays_desc->array[dev]);//get_array(arrays_desc, dev);
    void *host_p =(char*)host_buf + size * dev;
    void *dev_p = array_desc->deviceptr;
    e = cudaMemcpy(host_p, dev_p, size, cudaMemcpyDeviceToHost);
    cudaErrorCheck(e);
  }

  // reduce in node
  for(int dev = device->lb + device->step; dev <= device->ub; dev += device->step){
    reduce(host_buf,(char*) host_buf + size * dev, datatype, op);
  }
  ...
  // copy host to dev
  for(int dev = device->lb; dev <= device->ub; dev += device->step){
    _XACC_array_t *array_desc = &(arrays_desc->array[dev]);
    void *host_p = host_buf;
    void *dev_p = array_desc->deviceptr;
    e = cudaMemcpy(dev_p, host_p, size, cudaMemcpyHostToDevice);
    cudaErrorCheck(e);
  }
