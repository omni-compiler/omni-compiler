...
static void _XACC_reflect_sched_dim(_XACC_arrays_t *arrays_desc, int target_device, int target_dim){
  ...
  if(target_dim == 1){
    ...
    stream_pool++;
    // = (cudaStream_t*)_XMP_alloc(sizeof(cudaStream_t));
  ...
  if(1){//if((src != MPI_PROC_NULL && dst != MPI_PROC_NULL && (lo_buf_size / type_size) <= useSingleStreamLimit) || !useHostBuffer){
...
static void reflect_pack_start(_XMP_reflect_sched_t *reflect, size_t type_size)
{
  ...
  if(lo_send_array)
    _XACC_gpu_pack_vector_async(lo_send_buf, lo_send_array, 1, lo_width * reflect->blocklength, reflect->stride,
				type_size, *st_lo);
  if(hi_send_array)
    _XACC_gpu_pack_vector_async(hi_send_buf, hi_send_array, 1, hi_width * reflect->blocklength, reflect->stride,
  				type_size, *st_lo);
