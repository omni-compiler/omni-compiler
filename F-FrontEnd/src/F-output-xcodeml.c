...
/*
 * Check if the block is generated by FORALL/CONCURRENT DO and ignorable
 */
static int
is_ignorable_generated_block(const BLOCK_ENV block)
{
    ID id;

    if (!BLOCK_LOCAL_SYMBOLS(block)) {
        /* There is no index variable,
         * so this is not generated by compile_forall_header() */
        return FALSE;
    }
    ...
    /* There are only index variables,
     * so this is generated by compile_forall_header() and
     * All index variables are declared in the forall-header.
     */
    return TRUE;
}
...
outx_BLOCK_statement(int l, expv v)
{
    ...
    if (is_ignorable_generated_block(block)) {
        outx_body(l, EXPR_ARG1(v));
        return;
    }
