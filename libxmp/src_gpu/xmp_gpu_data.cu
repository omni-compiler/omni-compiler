...
extern "C" void _XMP_gpu_init_data_NOT_ALIGNED(_XMP_gpu_data_t **host_desc, _XMP_gpu_data_t **device_desc, void **device_addr, void *addr, size_t size) {
  _XMP_gpu_data_t *host_d = NULL;
  _XMP_gpu_data_t *device_d = NULL;

  // alloc desciptors
  host_d = (_XMP_gpu_data_t *)_XMP_alloc(sizeof(_XMP_gpu_data_t));
  _XMP_gpu_alloc((void **)&device_d, sizeof(_XMP_gpu_data_t));
  ...
  host_d->device_gpu_data_desc = device_d;
  ...
  *device_addr = host_d->device_addr;
  ...
  cudaMemcpy(device_d, host_d, sizeof(_XMP_gpu_data_t), cudaMemcpyHostToDevice);
}

extern "C" void _XMP_gpu_init_data_ALIGNED(_XMP_gpu_data_t **host_data_desc, _XMP_gpu_data_t **device_gpu_data_desc, void **device_addr, void *addr, _XMP_array_t *array_desc) {
  _XMP_gpu_data_t *host_d = NULL;
  _XMP_gpu_data_t *device_d = NULL;
  ...
  *host_data_desc = host_d;
  *device_gpu_data_desc = device_d;
  *device_addr = host_d->device_addr;
...
extern "C" void _XMP_gpu_finalize_data(_XMP_gpu_data_t *desc) {
  ...
  _XMP_gpu_free(desc->device_gpu_data_desc);
