...
void _xmp_gasnet_lock(_XMP_coarray_t* c, int position, int target_node){
  ...
  if(target_node == gasnet_mynode()){
    ...
    gasnet_AMRequestShort3(target_node, _XMP_GASNET_LOCK_REQUEST, (int)gasnet_mynode(), HIWORD(lockaddr), LOWORD(lockaddr));
  }
...
void _xmp_gasnet_unlock(_XMP_coarray_t* c, int position, int target_node){
  ...
  if(target_node == gasnet_mynode()){
    upcr_poll();
    _xmp_gasnet_do_unlock(gasnet_mynode(), lockaddr, (int *)&local_lockstate, (int *)&local_handoffarg);
  } else{
    local_lockstate = _XMP_LOCKSTATE_WAITING;
    ...
    */
    ...
    // only supports 64 bits arch.
    gasnet_AMRequestShort3(target_node, _XMP_GASNET_UNLOCK_REQUEST, (int)gasnet_mynode(), HIWORD(lockaddr), LOWORD(lockaddr));
  }
