...
static void show_all(_XMP_array_t *p)
{
   ...
   for(r=0; r<size; r++){
      if(r==rank){
         ...
         printf("dim=%d, type=%d, type_size=%d\n", p->dim, p->type, (int)(p->type_size));
         printf("align: [");
         for(i=0; i<p->dim; i++){
...
#if 0
static int g_to_l(_XMP_array_t *ap, int dim, int index)
{
   _XMP_template_t *t;
   ...
   if(tdim >= 0){
      ...
      case _XMP_N_DIST_BLOCK:
         tindex = index+ap->info[dim].align_subscript - t->info[tdim].ser_lower;
         lindex = tindex % chunk->par_chunk_width;
         break;
      case _XMP_N_DIST_CYCLIC:
      case _XMP_N_DIST_BLOCK_CYCLIC:
         tindex = index+ap->info[dim].align_subscript - t->info[tdim].ser_lower;
         ...
         break;
      case _XMP_N_DIST_GBLOCK:
         tindex = index+ap->info[0].align_subscript - t->info[tdim].ser_lower;
   ...
   if(p == 0){
      return lindex-ap->info[dim].align_subscript;
   } else {
      return lindex;
   }
}
...
static void xmp_transpose_no_opt(_XMP_array_t *dst_d, _XMP_array_t *src_d, int opt)
{
   ...
   /* duplicate */
   {
      ...
      if(dst_d->info[0].align_manner == _XMP_N_ALIGN_BLOCK ||
         ...
         nodes = dst_d->align_template->onto_nodes;
         dist_dim++;
      }
...
static void xmp_transpose_alltoall(_XMP_array_t *dst_d, _XMP_array_t *src_d, int opt, int dist_dim)
{
   ...
   send_buf = (char*)_XMP_alloc(src_d->type_size*src_d->info[0].alloc_size*src_d->info[1].alloc_size);
   if(_XMPF_running){           /* Fortran */
      if(dist_dim == 0 && dst_d->info[dist_dim].shadow_size_lo == 0 && dst_d->info[dist_dim].shadow_size_hi == 0){
         ...
         recv_buf = (char*)_XMP_alloc(dst_d->type_size*buf_size);
      }
   } else {
      ...
      if(dist_dim == 0){
         for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
            for(j=0; j<dst_w; j++){
               ...
               for(i=src_d->info[0].local_lower; i<=src_d->info[0].local_upper; i++){
                  memcpy(send_buf+(k*offset_size+(i-src_d->info[0].local_lower)*dst_w+j)*src_d->type_size,
                         addr_p+(l*src_d->info[0].alloc_size+i)*src_d->type_size, src_d->type_size);
               }
            }
         }
      } else {
         for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
            for(j=src_d->info[1].local_lower; j<=src_d->info[1].local_upper; j++){
               for(i=0; i<dst_w; i++){
                  ...
                  memcpy(send_buf+(k*offset_size+i*src_w+j-src_d->info[1].local_lower)*src_d->type_size,
                         addr_p+(j*src_d->info[0].alloc_size+l)*src_d->type_size,
                         src_d->type_size);
               }
            }
         }
         ...
         for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
            for(j=src_d->info[0].local_lower; j<=src_d->info[0].local_upper; j++){
               for(i=0; i<dst_w; i++){
                  ...
                  memcpy(send_buf+(k*offset_size+i*src_w+j-src_d->info[0].local_lower)*src_d->type_size,
                         addr_p+(j*src_d->info[1].alloc_size+l)*src_d->type_size,
                         src_d->type_size);
               }
            }
         }
         ...
         for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
            for(j=0; j<dst_w; j++){
               ...
               for(i=src_d->info[1].local_lower; i<=src_d->info[1].local_upper; i++){
                  memcpy(send_buf+(k*offset_size+(i-src_d->info[1].local_lower)*dst_w+j)*src_d->type_size,
                         addr_p+(l*src_d->info[1].alloc_size+i)*src_d->type_size, src_d->type_size);
               }
            }
         }
      }
   ...
   for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
      if(k == _XMP_get_execution_nodes()->comm_rank){
         ...
         printf("send_buf (%2d)\n", k);
         for(j=0; j<_XMP_get_execution_nodes()->comm_size; j++){
            a = (int*)(send_buf+j*offset_size*src_d->type_size);
            printf(" to %d: ", j);
   ...
   /* communication */
   MPI_Alltoall(send_buf, src_w*dst_w*src_d->type_size, MPI_CHAR,
                recv_buf, src_w*dst_w*src_d->type_size, MPI_CHAR, *((MPI_Comm*)_XMP_get_execution_nodes()->comm));
   ...
   if(_XMPF_running){           /* Fortran */
      if(dist_dim == 1){
         for(j=dst_d->info[1].local_lower; j<=dst_d->info[1].local_upper; j++){
            for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
               memcpy(addr_p+(dst_d->info[0].alloc_size*j+src_w*k+dst_d->info[0].local_lower)*src_d->type_size,
                      recv_buf+(k*src_w*dst_w+(j-dst_d->info[1].local_lower)*src_w)*src_d->type_size,
                      src_w*src_d->type_size);
            }
         }
      } else if(dst_d->info[0].shadow_size_lo != 0 || dst_d->info[0].shadow_size_hi != 0){
         for(j=dst_d->info[1].local_lower; j<=dst_d->info[1].local_upper; j++){
            memcpy(addr_p+(dst_d->info[0].alloc_size*j+dst_d->info[0].local_lower)*src_d->type_size,
                   recv_buf+((j-dst_d->info[1].local_lower)*dst_w)*src_d->type_size,
                   src_w*src_d->type_size);
         }
      }
   } else {                     /* C */
      if(dist_dim == 0){
         for(j=dst_d->info[0].local_lower; j<=dst_d->info[0].local_upper; j++){
            for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
               memcpy(addr_p+(dst_d->info[1].alloc_size*j+src_w*k+dst_d->info[1].local_lower)*src_d->type_size,
                      recv_buf+(k*src_w*dst_w+(j-dst_d->info[0].local_lower)*src_w)*src_d->type_size,
                      src_w*src_d->type_size);
            }
         }
      } else if(dst_d->info[1].shadow_size_lo != 0 || dst_d->info[1].shadow_size_hi != 0){
         for(j=dst_d->info[0].local_lower; j<=dst_d->info[0].local_upper; j++){
            memcpy(addr_p+(dst_d->info[1].alloc_size*j+dst_d->info[1].local_lower)*src_d->type_size,
                   recv_buf+((j-dst_d->info[0].local_lower)*dst_w)*src_d->type_size,
                   src_w*src_d->type_size);
         }
      }
   }
...
void xmp_transpose(void *dst_p, void *src_p, int opt)
{
   ...
   } else if(_XMPF_running && same_nodes && same_align && regular && !duplicate && dist_num == 1 && dist_dim == 1){
   ...
   /* } else if(same_nodes && same_align && dist_num == 1){ */
   /*    /\* collective MPI communication transpose *\/ */
...
static int l2g(_XMP_array_t *array_d, int dim, int idx)
{
   ...
   case _XMP_N_ALIGN_BLOCK:
      ...
      ret = lidx+chunk->onto_nodes_info->rank*chunk->par_chunk_width;
      break;
   case _XMP_N_ALIGN_CYCLIC:
      chunk = &(array_d->align_template->chunk[array_d->info[dim].align_template_index]);
      if(chunk->onto_nodes_info->rank == 0) lidx += array_d->info[dim].align_subscript;
      ret = chunk->onto_nodes_info->size*lidx+chunk->onto_nodes_info->rank;
      break;
   case _XMP_N_ALIGN_BLOCK_CYCLIC:
      chunk = &(array_d->align_template->chunk[array_d->info[dim].align_template_index]);
      if(chunk->onto_nodes_info->rank == 0) lidx += array_d->info[dim].align_subscript;
      ret = chunk->onto_nodes_info->size*chunk->par_width*(lidx/chunk->par_width)
         + chunk->onto_nodes_info->rank*chunk->par_width+lidx%chunk->par_width;
      break;
   ...
   ret += array_d->info[dim].ser_lower-array_d->info[dim].align_subscript;
   return ret;
}
...
static void xmp_matmul_no_opt(_XMP_array_t *x_d, _XMP_array_t *a_d, _XMP_array_t *b_d)
{
   ...
   if(x_d->is_allocated){
      a_buf = (char*)_XMP_alloc(x_d->info[0].ser_size*x_d->info[1].ser_size*x_d->type_size);
      b_buf = (char*)_XMP_alloc(x_d->info[0].ser_size*x_d->info[1].ser_size*x_d->type_size);
   } else {
      a_buf = NULL;
      b_buf = NULL;
   }
   ...
   if(_XMPF_running){           /* Fortran */
      ...
      MPI_Barrier(*(MPI_Comm*)(_XMP_get_execution_nodes()->comm));
#endif
      for(i=0; i<proc_size(x_d, 1); i++){
         for(j=0; j<proc_size(x_d, 0); j++){
...
#ifdef DEBUG
            printf("recv: %d -> %d: %d\n", src_rank, _XMP_get_execution_nodes()->comm_rank, b_recv_size[j]);
#endif
   ...
   if(_XMPF_running){           /* Fortran */
      ...
      for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
         if(k == _XMP_get_execution_nodes()->comm_rank){
            printf("b_buf_size (%2d):", k);
            for(j=0; j<_XMP_get_execution_nodes()->comm_size; j++){
               printf("%2d ", b_recv_size[j]);
            }
            printf("\n");
            fflush(stdout);
         }
         ...
         for(j=x_d->info[0].local_lower; j<=x_d->info[0].local_upper; j++){
            ...
            for(k=0; k<a_d->info[1].ser_size; k++){
               ...
               char *a_p = (char*)(a_buf+a_recv_pos[a2e[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)]]
                                   +(a_offset[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)])*a_d->type_size);
               ...
               case _XMP_N_TYPE_INT:
                  {
                     ...
                     *x_var += *a_var * *b_var;
#ifdef DEBUG
                     if(j==x_d->info[0].local_lower+0 && i==x_d->info[1].local_lower+2 &&
                        _XMP_get_execution_nodes()->comm_rank == 0){
                        printf("%4d x %4d: ar=%d offset=%d: br=%d offset=%d (%d,%d)->(%d,%d)\n", *a_var, *b_var,
                               a2e[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)],
                               a_offset[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)]-1,
                               b2e[g2p_array(b_d,k+b_d->info[0].ser_lower,gi)],
                               b_idx0+b_offset[b2e[g2p_array(b_d,k+b_d->info[0].ser_lower,gi)]],
                               j, i, l2g(x_d, 0, j), l2g(x_d, 1, i));
                     }
#endif
                  }
                  break;
               ...
               case _XMP_N_TYPE_FLOAT_COMPLEX:
                  {
                     _Complex *x_var = (_Complex*)x_p;
                     _Complex *a_var = (_Complex*)a_p;
                     _Complex *b_var = (_Complex*)b_p;
                     *x_var += *a_var * *b_var;
                  }
                  break;
      ...
      for(k=0; k<_XMP_get_execution_nodes()->comm_size; k++){
         if(k == _XMP_get_execution_nodes()->comm_rank){
            printf("a_buf_size (%2d):", k);
            for(j=0; j<_XMP_get_execution_nodes()->comm_size; j++){
               printf("%2d ", a_recv_size[j]);
            }
            printf("\n");
            fflush(stdout);
         }
      ...
      for(j=x_d->info[0].local_lower; j<=x_d->info[0].local_upper; j++){
         ...
         for(i=x_d->info[1].local_lower; i<=x_d->info[1].local_upper; i++){
            ...
            for(k=0; k<a_d->info[1].ser_size; k++){
               ...
               _XMP_align_local_idx((long long int)(k+b_d->info[0].ser_lower), &b_idx0, b_d, 0, &b_rank0);
               char *a_p = (char*)(a_buf+a_recv_pos[a2e[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)]]
                                   +(a_idx1+a_offset[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)])*a_d->type_size);
               char *b_p = (char*)(b_buf+b_recv_pos[b2e[g2p_array(b_d,k+b_d->info[0].ser_lower,gi)]]
               ...
               case _XMP_N_TYPE_INT:
                  {
                     ...
                     *x_var += *a_var * *b_var;
#ifdef DEBUG
                     if(j==x_d->info[0].local_lower+7 && i==x_d->info[1].local_lower+7 &&
                        _XMP_get_execution_nodes()->comm_rank == 3){
                        printf("%4d x %4d: ar=%d offset=%d: br=%d offset=%d (%d,%d)->(%d,%d)\n", *a_var, *b_var,
                               a2e[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)],
                               a_idx1+a_offset[a2e[g2p_array(a_d,gj,k+a_d->info[1].ser_lower)]],
                               b2e[g2p_array(b_d,k+b_d->info[0].ser_lower,gi)],
                               b_offset[b2e[g2p_array(b_d,k+b_d->info[0].ser_lower,gi)]]-1,
                               j, i, l2g(x_d, 0, j), l2g(x_d, 1, i));
                     }
#endif
                  }
