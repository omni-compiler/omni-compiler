...
void _XCALABLEMP_bcast_EXEC_OMITTED(void *addr, int count, size_t datatype_size) {
  // setup type
  MPI_Datatype mpi_datatype;
  MPI_Type_contiguous(datatype_size, MPI_BYTE, &mpi_datatype);
  MPI_Type_commit(&mpi_datatype);
  ...
  // bcast
  _XCALABLEMP_nodes_t *bcast_nodes = _XCALABLEMP_get_execution_nodes();
  MPI_Bcast(addr, count, mpi_datatype, 0, *(bcast_nodes->comm));
}

void _XCALABLEMP_bcast_EXEC_GLOBAL(void *addr, int count, size_t datatype_size, int from_lower, int from_upper, int from_stride) {
  // check <from-ref>
  _XCALABLEMP_validate_nodes_ref(&from_lower, &from_upper, &from_stride, _XCALABLEMP_world_size);
  if (_XCALABLEMP_M_COUNT_TRIPLETi(from_lower, from_upper, from_stride) != 1)
    _XCALABLEMP_fatal("multiple source nodes indicated in bcast directive");
...
void _XCALABLEMP_bcast_EXEC_NODES(void *addr, int count, size_t datatype_size, _XCALABLEMP_nodes_t *from_nodes, ...) {
  if (from_nodes == NULL) _XCALABLEMP_fatal("error on broadcast, cannot access to the source node");
