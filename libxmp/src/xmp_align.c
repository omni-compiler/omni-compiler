                            ...
                            long long align_subscript, int *temp0)
{
  ...
  if(template->is_owner){
    ...
    ai->local_lower  = 0;  // array lower is always 0 in C
...
void _XMP_init_array_nodes(_XMP_array_t *array)
{
  ...
  if (array->is_shrunk_template){
    array->array_nodes =
      _XMP_create_nodes_by_template_ref(align_template, align_template_shrink, align_template_lower,
					align_template_upper, align_template_stride);
  }else{
