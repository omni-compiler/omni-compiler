...
void _XMP_fjrdma_shortcut_put(const int target, const uint64_t dst_point, const uint64_t src_point,
			      const _XMP_coarray_t *dst_desc, const _XMP_coarray_t *src_desc, const size_t transfer_size)
{
  ...
  uint64_t raddr = (uint64_t)dst_desc->addr[target] + dst_point;
  uint64_t laddr = (uint64_t)src_desc->addr[_XMP_world_rank] + src_point;

  FJMPI_Rdma_put(target, FJRDMA_TAG, raddr, laddr, transfer_size, _XMP_FLAG_NIC);
  _num_of_puts++;
}

// Both src and dst arrays are continuous.
static void _fjrdma_continuous_put(const int target, const uint64_t dst_point, const uint64_t src_point,
...
static void _FX10_Rdma_mput(int target, uint64_t *raddrs, uint64_t *laddrs, 
			    size_t *lengths, int stride, size_t transfer_elmts)
{
  if(stride == 0){
    for(int i=0;i<transfer_elmts;i++)
      FJMPI_Rdma_put(target, FJRDMA_TAG, raddrs[i], laddrs[i], lengths[i], _XMP_FLAG_NIC);
  }
  else{
    for(int i=0;i<transfer_elmts;i++)
      FJMPI_Rdma_put(target, FJRDMA_TAG, raddrs[0]+i*stride, laddrs[0]+i*stride, lengths[0], _XMP_FLAG_NIC);
  }
}
#endif
				   ...
				   size_t* lengths, const size_t transfer_elmts, const size_t elmt_size)
{
  if(transfer_elmts <= FJRDMA_MAX_MPUT){
    ...
    FJMPI_Rdma_mput(target, FJRDMA_TAG, raddrs, laddrs, lengths, 0, transfer_elmts, _XMP_FLAG_NIC);
    _num_of_puts++;
#elif OMNI_TARGET_CPU_FX10
    _FX10_Rdma_mput(target, raddrs, laddrs, lengths, 0, transfer_elmts);
    _num_of_puts += transfer_elmts;
#endif
  }
  else{
    int times      = transfer_elmts / FJRDMA_MAX_MPUT + 1;
    int rest_elmts = transfer_elmts - FJRDMA_MAX_MPUT * (times - 1);
    size_t trans_elmts;

    for(int i=0;i<times;i++){
      trans_elmts = (i != times-1)? FJRDMA_MAX_MPUT : rest_elmts;
#ifdef OMNI_TARGET_CPU_KCOMPUTER
      FJMPI_Rdma_mput(target, FJRDMA_TAG, &raddrs[i*FJRDMA_MAX_MPUT], &laddrs[i*FJRDMA_MAX_MPUT],
...
// e.g. a[0:100:3]:[2] = b;
static void _fjrdma_scalar_mput(const int target, const uint64_t dst_point, const uint64_t src_point, 
				const _XMP_array_section_t *dst_info, const int dst_dims,
				const _XMP_coarray_t *dst_desc, const _XMP_coarray_t *src_desc, 
				const void *src, const size_t transfer_elmts)
/* If a local array is a coarray, src_desc != NULL. */
{
  uint64_t raddr = (uint64_t)dst_desc->addr[target] + dst_point;
  uint64_t laddr;
...
{
  return array[0].distance * array[0].elmts;
}

static void _fjrdma_NON_continuous_put_1dim_same_stride(const int target, uint64_t raddr, uint64_t laddr,
							const size_t transfer_elmts, const int stride, size_t elmt_size)
{
  ...
  */
  if(transfer_elmts <= FJRDMA_MAX_MPUT){
#ifdef OMNI_TARGET_CPU_KCOMPUTER
    FJMPI_Rdma_mput(target, FJRDMA_TAG, &raddr, &laddr,
		    &elmt_size, stride, transfer_elmts, _XMP_FLAG_NIC);
    _num_of_puts++;
#elif OMNI_TARGET_CPU_FX10
    _FX10_Rdma_mput(target, &raddr, &laddr, &elmt_size, stride, transfer_elmts);
    _num_of_puts += transfer_elmts;
#endif
  }
  else{
    ...
    for(int i=0;i<times;i++){
      ...
      trans_elmts = (i != times-1)? FJRDMA_MAX_MPUT : rest_elmts;
#ifdef OMNI_TARGET_CPU_KCOMPUTER
      FJMPI_Rdma_mput(target, FJRDMA_TAG, &tmp_raddr, &tmp_laddr,
		      &elmt_size, stride, trans_elmts, _XMP_FLAG_NIC);
      _num_of_puts++;
#elif OMNI_TARGET_CPU_FX10
      _FX10_Rdma_mput(target, &tmp_raddr, &tmp_laddr, &elmt_size, stride, trans_elmts);
      _num_of_puts += trans_elmts;
#endif
    }
  }
...
static void _fjrdma_NON_continuous_put_general(const int target, uint64_t raddr, uint64_t laddr,
					       ...
					       const int dst_dims, const int src_dims, size_t elmt_size)
{
  ...
  // Set parameters for FJMPI_Rdma_mput
  _XMP_set_coarray_addresses(raddr, dst_info, dst_dims, transfer_elmts, raddrs);
  _XMP_set_coarray_addresses(laddr, src_info, src_dims, transfer_elmts, laddrs);
  for(int i=0;i<transfer_elmts;i++) lengths[i] = elmt_size;
...
static void _fjrdma_NON_continuous_put(const int target, const uint64_t dst_point, const uint64_t src_point,
				       const _XMP_array_section_t *dst_info, const _XMP_array_section_t *src_info,
...
/* If a local array is a coarray, src_desc != NULL. */
{
  ...
  if(dst_dims == 1 && src_dims == 1 && dst_info[0].stride == src_info[0].stride){
    _fjrdma_NON_continuous_put_1dim_same_stride(target, raddr, laddr, transfer_elmts,
						dst_info[0].stride*elmt_size, elmt_size);
  }
  else{
    _fjrdma_NON_continuous_put_general(target, raddr, laddr, transfer_elmts, 
...
void _XMP_fjrdma_put(const int dst_continuous, const int src_continuous, const int target, const int dst_dims, const int src_dims, 
		     ...
		     void *src, const _XMP_coarray_t *src_desc, const int dst_elmts, const int src_elmts)
{
  ...
  if(dst_elmts == src_elmts){
    if(dst_continuous == _XMP_N_INT_TRUE && src_continuous == _XMP_N_INT_TRUE){
      _fjrdma_continuous_put(target, dst_point, src_point, dst_desc, src_desc, src, transfer_size);
    }
    else{
      _fjrdma_NON_continuous_put(target, dst_point, src_point, dst_info, src_info, dst_dims, src_dims, dst_desc, src_desc, 
				 src, dst_elmts);
    }
  }
  else{
    if(src_elmts == 1){
      _fjrdma_scalar_mput(target, dst_point, src_point, dst_info, dst_dims, dst_desc, src_desc, src, dst_elmts);
    }
    else{
...
void _XMP_fjrdma_shortcut_get(const int target, const uint64_t dst_point, const uint64_t src_point,
			      const _XMP_coarray_t *dst_desc, const _XMP_coarray_t *src_desc,
			      const size_t transfer_size)
{
  ...
  FJMPI_Rdma_get(target, FJRDMA_TAG, raddr, laddr, transfer_size, _XMP_FLAG_NIC);

  // To complete the above get operation.
  while(FJMPI_Rdma_poll_cq(_XMP_SEND_NIC, &_cq) != FJMPI_RDMA_NOTICE);
}

static void _fjrdma_continuous_get(const int target, const uint64_t dst_point, const uint64_t src_point,
...
static void _fjrdma_NON_continuous_get(const int target, const uint64_t dst_point, const uint64_t src_point,
				       const _XMP_array_section_t *dst_info, const _XMP_array_section_t *src_info,
...
/* If a local array is a coarray, dst_desc != NULL. */
{
  ...
  _XMP_set_coarray_addresses(raddr, src_info, src_dims, transfer_elmts, raddrs);

  // To complete put operations before the following get operation.
  _XMP_fjrdma_sync_memory();

  if(transfer_elmts <= FJRDMA_MAX_MGET){
    for(int i=0;i<transfer_elmts;i++)
      FJMPI_Rdma_get(target, FJRDMA_TAG, raddrs[i], laddrs[i], elmt_size, _XMP_FLAG_NIC);
    ...
    int times      = transfer_elmts / FJRDMA_MAX_MGET + 1;
    int rest_elmts = transfer_elmts - FJRDMA_MAX_MGET * (times - 1);

    for(int i=0;i<times;i++){
      size_t trans_elmts = (i != times-1)? FJRDMA_MAX_MGET : rest_elmts;
      for(int j=0;j<trans_elmts;j++)
	FJMPI_Rdma_get(target, FJRDMA_TAG, raddrs[j+i*FJRDMA_MAX_MGET], laddrs[j+i*FJRDMA_MAX_MGET], 
				   ...
				   const size_t transfer_elmts, const size_t elmt_size)
{
  ...
  if(transfer_elmts <= FJRDMA_MAX_MGET){
    for(int i=0;i<transfer_elmts;i++)
      FJMPI_Rdma_get(target, FJRDMA_TAG, raddr, laddrs[i], elmt_size, _XMP_FLAG_NIC);
    ...
    for(int i=0;i<times;i++){
      ...
      for(int j=0;j<trans_elmts;j++)
        FJMPI_Rdma_get(target, FJRDMA_TAG, raddr, laddrs[j+i*FJRDMA_MAX_MGET],
                       elmt_size, _XMP_FLAG_NIC);
...
static void _fjrdma_scalar_mget(const int target, const uint64_t dst_point, const uint64_t src_point,
				const _XMP_array_section_t *dst_info, const int dst_dims,
				const _XMP_coarray_t *dst_desc, const _XMP_coarray_t *src_desc,
				const void *dst, const size_t transfer_elmts)
/* If a local array is a coarray, dst_desc != NULL. */
{
  uint64_t raddr = (uint64_t)src_desc->addr[target] + src_point;
  uint64_t laddr, laddrs[transfer_elmts];
  size_t elmt_size = dst_desc->elmt_size;

  if(dst_desc == NULL)
    laddr = FJMPI_Rdma_reg_mem(_XMP_TEMP_MEMID, (char *)dst, elmt_size*transfer_elmts) + dst_point;
  else
    laddr = (uint64_t)dst_desc->addr[_XMP_world_rank] + dst_point;
...
void _XMP_fjrdma_get(const int src_continuous, const int dst_continuous, const int target, const int src_dims, const int dst_dims, 
		     const _XMP_array_section_t *src_info, const _XMP_array_section_t *dst_info, const _XMP_coarray_t *src_desc, 
		     void *dst, const _XMP_coarray_t *dst_desc, const int src_elmts, const int transfer_elmts)
{
  uint64_t dst_point = (uint64_t)_XMP_get_offset(dst_info, dst_dims);
  uint64_t src_point = (uint64_t)_XMP_get_offset(src_info, src_dims);
  ...
  if(src_elmts == transfer_elmts){
    if(dst_continuous == _XMP_N_INT_TRUE && src_continuous == _XMP_N_INT_TRUE){
      _fjrdma_continuous_get(target, dst_point, src_point, dst, dst_desc, src_desc, transfer_size);
    }
    else{
      _fjrdma_NON_continuous_get(target, dst_point, src_point, dst_info, src_info,
