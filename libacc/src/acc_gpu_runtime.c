...
static void init_device(int dev_num){ //0-based
  ...
  contexts[get_actual_device_num(dev_num)].memory_map = _ACC_gpu_init_data_table();
...
{
  _ACC_DEBUG("get_current_memory_map\n")
  void *memory_map = contexts[get_actual_device_num(current_device_num)].memory_map;
  if(memory_map == NULL){
	_ACC_gpu_init_device_if_not_inited(current_device_num);
  }
  return contexts[get_actual_device_num(current_device_num)].memory_map;
}
