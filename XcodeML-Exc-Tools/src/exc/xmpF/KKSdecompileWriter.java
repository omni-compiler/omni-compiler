...
public class KKSdecompileWriter extends PrintWriter {
  private XobjectFile _env = null;

  public KKSdecompileWriter(Writer out, XobjectFile env) {
    super(out);
    _env = env;
  }

  public void printKKSfunc(XobjectDef def, Ident deviceFuncId) {
...
//      break;
    ...
    case FUNCTION_DEFINITION: // for LAMBDA functions
      {
	println();
	
	Ident id = (Ident)v.getArg(0);
  ...
  private String makeTypeName(Xtype type) {
    ...
    switch (type.getKind()) {
      ...
      case Xtype.STRUCT:
        {
          if (!((StructType)type).isClass()) typename += "struct ";
          if (((StructType)type).getTagNames() != null) {
	    typename += ((StructType)type).getTagNames().getString();
          } else {
  ...
  private void printArgList(Xobject v) {
    ...
    if (v != null) {
      for (XobjArgs a = v.getArgs(); a != null; a = a.nextArgs()) {
	if (a.getArg().Opcode() == Xcode.LIST){ // C++ array constant
	  print("{");
	  for (XobjArgs aa = a.getArg().getArgs(); aa != null; aa = aa.nextArgs()){
	    print(aa.getArg());
	    if (aa.nextArgs() != null) print(",");
	  }
	  print("}");
	}
	else {
