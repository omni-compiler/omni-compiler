...
public class XMPobjectsRef {
  ...
  public static XMPobjectsRef parseDecl(Xobject decl, XMPglobalDecl globalDecl,
					PragmaBlock pb){
    if(decl == null || decl.hasNullArg()) return null;
    XMPobjectsRef objRef = new XMPobjectsRef();
    objRef.parse(decl, globalDecl, pb);
    return objRef;
  }
  ...
  void parse(Xobject decl, XMPglobalDecl globalDecl, PragmaBlock pb) {
    if (decl.getArg(0) == null) {
      ...
      refObject = globalDecl.getXMPobject(refName, pb);
      if (refObject == null) {
	XMP.error(pb.getLineNo(),"cannot find objects '" + refName + "'");
	return;
      }
	...
	if (!subscriptList.isEmptyList() && subscripts.size() != refObject.getDim()){
	  XMP.error(pb.getLineNo(), "wrong number of subscripts");
	  return;
	}
    ...
    // allocate DescId
    descId = pb.getParent().declLocalIdent(globalDecl.genSym("XMP_REF_"+refName), Xtype.voidPtrType);
  }
  ...
  public Block buildConstructor(XMPglobalDecl globalDecl){
    ...
    case XMPobject.NODES:
      f = globalDecl.declExternFunc("_XMP_ref_nodes_alloc");
      break;
    case XMPobject.TEMPLATE:
      f = globalDecl.declExternFunc("_XMP_ref_templ_alloc");
      break;
    default:
      XMP.fatal("bad object for ref");
      return null;
    }
    Xobject args = Xcons.List(descId.getAddr(),refObject.getDescId().Ref(),
			      Xcons.IntConstant(subscripts.size()));
    bb.add(f.callSubroutine(args));
    f = globalDecl.declExternFunc("_XMP_ref_set_dim_info");
	...
	if (d_info.hasUpper()){
	  ...
	  if (refObject.getKind() == XMPobject.NODES){
	    decl_upper = ((XMPnodes)refObject).getSizeAt(i);
	    if (decl_upper == null){
	      decl_upper = DUMMY;
	      noUB_flag = true;
	    }
    ...
    f = globalDecl.declExternFunc("_XMP_ref_init");
