...
public class omompx {
  ...
  private static void usage() {
    final String[] lines = {
        ...
        "           <input XcodeML file>", "           [-o <output reconstructed XcodeML file>]", "",
        ...
        "  -fopenmp-only-target  enable OpenMP only target translation.", "  -fcoarry[=suboption]",
        "                        enable coarray translation optionally with a suboption.",
        "  -fnocoarry            pass without coarray translation (default for C).", "  -fcoarry-no-use-statement",
        ...
        "  -decomp               output decompiled source code.", "  -silent               no output.",
        "  -rename_main=NAME", "                        rename the main function NAME.", "", " Debug Options:",
        ...
        "  -domp                 enable output OpenMP translation debug message.", " Profiling Options:",
        ...
        "  -enable-threads       enable 'threads' clause", "  -enable-gpu           enable xmp-dev directive/clauses",
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (xmpf && (xmpf_skipCafMode || !XmOption.isCoarray())) {
      System.out.println(
          "<SKIP-CAF MODE> XMP/F Coarray translator is " + "bypassed for " + xobjFile.getSourceFileName() + ".");
    }

    if (xmpf && (!xmpf_skipCafMode && XmOption.isCoarray())) {

      // Coarray Fortran pass#3
      exc.xmpF.XMPtransCoarray caf_translator3 = new exc.xmpF.XMPtransCoarray(xobjFile, 3, coarray_suboption,
          xmpf_onlyCafMode);
      xobjFile.iterateDef(caf_translator3);

      // Coarray Fortran pass#4
      exc.xmpF.XMPtransCoarray caf_translator4 = new exc.xmpF.XMPtransCoarray(xobjFile, 4, coarray_suboption,
          xmpf_onlyCafMode);
    ...
    if (xmpf && xmpf_onlyCafMode) {
      System.out.println(
          "<ONLY-CAF MODE> XMP/F gloval-view translator is " + "bypassed for " + xobjFile.getSourceFileName() + ".");
    }
    ...
    if (xmpf && (!xmpf_skipCafMode && XmOption.isCoarray())) {
      ...
      exc.xmpF.XMPtransCoarray caf_translator1 = new exc.xmpF.XMPtransCoarray(xobjFile, 1, coarray_suboption,
          xmpf_onlyCafMode);
      ...
      exc.xmpF.XMPtransCoarray caf_translator2 = new exc.xmpF.XMPtransCoarray(xobjFile, 2, coarray_suboption,
          xmpf_onlyCafMode);
    ...
    decompile(lang, xobjFile, silent, outXmlFile, maxColumns, outputDecomp, dump, srcPath, baseName, dir, null);
  }
