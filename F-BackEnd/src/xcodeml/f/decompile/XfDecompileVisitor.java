...
public class XfDecompileVisitor extends RVisitorBase
{
  ...
  int operator_priority(String operator){
    /* NOTE:
     *  This function can not distinguish the defined unary operator between defined binary operator.
     *  And the same applies to the unary minus operator and the binary minus operator.
     */
  ...
  int operator_priority(IXbfDefModelExprChoice expr){
    ...
    if(expr instanceof  XbfUnaryMinusExpr)
    ...
    if(expr instanceof XbfLogNotExpr) return PRIO_NOT;
    ...
    if(expr instanceof XbfUserBinaryExpr) return PRIO_DEFINED_BINARY;
    if(expr instanceof XbfUserUnaryExpr) return PRIO_DEFINED_UNARY;
...
//        }
    ...
    {
        // DONE: XbfDivExpr
        if (_writeBinaryExpr(visitable.getDefModelExpr1(),
                             visitable.getDefModelExpr2(),
                             "/",
                             checkBinaryExprRequireGrouping(visitable)) == false) {
            return false;
        }
    ...
    public boolean enter(XbfFpointerAssignStatement visitable)
    {
        ...
        if (_writeBinaryExpr((IXbfDefModelExprChoice)visitable.getDefModelLValue(),
                             visitable.getDefModelExpr(), "=>", false) == false) {
            return false;
        }
    ...
    private boolean checkBinaryExprRequireGrouping(IXbfDefModelExprChoice expr) {
        IRNode parent = expr.rGetParentRNode();

        if (!(parent instanceof XbfUnaryMinusExpr)
            && !(parent instanceof XbfUserUnaryExpr)
            && !(parent instanceof XbfLogNotExpr))
            return false;

        if (parent instanceof IXbfDefModelExprChoice) {
            return operator_priority((IXbfDefModelExprChoice)parent) > operator_priority(expr);
        } else {
            return false;
        }
    }
